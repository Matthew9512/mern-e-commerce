...
export const filterBy = (sortByParams, productsQuery) => {
   ...
   if (sortByParams === 'price-up') filteredData = productsQuery.data.slice().sort((a, b) => +a.price - +b.price);
   // return new arr filtered by price down
   if (sortByParams === 'price-down') filteredData = productsQuery.data.slice().sort((a, b) => +b.price - +a.price);
   // return new arr filtered by name
   if (sortByParams === 'name') filteredData = productsQuery.data.slice().sort((a, b) => a.name.localeCompare(b.name));
   // return new arr filtered by active discount
   if (sortByParams === 'discount') filteredData = productsQuery.data.filter((product) => product.discount);
